...
*** runtime
- backport "custom" block, useful for Bignum and Int32/Int64 (3.00)

*** stdlib
- backport Int32.ml and Int64.ml (depends on custom block) (3.00)

*** compilers
- port more arch x86_64(amd64)/arm64(aarch64)/riscv(and riscv64)
...
# leverage bench and examples/
# add amd64/arm64/riscv
# and even use goken9cc toolchain instead of gcc -m32 later!
# compile efuns on plan9!

** Later

*** try 'comptime' a la zig but in ocaml? better than metaocaml?
need ocaml interpreter in the compiler? need reify AST to the interpreter?
  special builtins for compile-time reflection?
can implement deriving with it? using the same syntax?!!
can implement let* with it?
...
*** add Float.pi, from 4.08
and delete it after from Common.Float submodule

*** delete camlprim0.8 once done
restore some delete in pcc? in ocaml-light? in
the previously installed /usr/local/bin/ocamlc?

*** fix clock.ml buggy timer thread
need sigvtalarm implemented?
...
*** does -thread imply the use of ThreadUnix? backport the feature
does not look like, it was probably a feature added later, 
so we should backport it! 

or maybe the functionality of ThreadUnix has been merged in Unix
itself?
...
*** LATER finish draw port so can then port ocaml-elm-playground to also
...
*** use capabilities in ocaml-light code itself? useful? most
should be pure anyway no? prevent some bootstrapping?

makes the code clearer? to know where we save to files, where we
call programs, etc.?
good way to better understand the code? to do some simple small
refactoring (adding caps parameters)
...
*** better dynlink
https://github.com/ocaml/ocaml/pull/1063
