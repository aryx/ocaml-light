...
`.align` semantics, `.L` local labels, and `.quad`/`.short`/`.byte`/`.double`/
`.space`/`.ascii`/`.globl` directives are all identical between GNU as (ELF
arm64) and LLVM's integrated assembler (Mach-O arm64) -- no changes needed
there, unlike i386's ELF-vs-Solaris split.
